    ...
    public static class Overclock
    {
        ...
        public static void Reactor(IMyTerminalBlock block, float value, bool find)
        {
            if (find)
            {
                ...
                {
                    if (s.FatBlock is IMyReactor) return true;
                    return false;
                });
        ...
        public static void GasGenerator(IMyTerminalBlock block, float value, bool find)
        {
            if (find)
            {
                ...
                block.CubeGrid.GetBlocks(Blocks, (s) =>
                {
                    if (s.FatBlock is IMyGasGenerator) return true;
                    return false;
                });
                foreach (IMySlimBlock b in Blocks)
                {
        ...
        public static void Gyro(IMyTerminalBlock block, float value, bool find)
        {
            if (find)
            {
                ...
                block.CubeGrid.GetBlocks(Blocks, (s) =>
                {
                    if (s.FatBlock is IMyGyro) return true;
                    return false;
                });
                foreach (IMySlimBlock b in Blocks)
                {
        ...
        public static void Thrust(IMyTerminalBlock block, float value, bool find)
        {
            if (find)
            {
                ...
                block.CubeGrid.GetBlocks(Blocks, (s) =>
                {
                    if (s.FatBlock is IMyThrust) return true;
                    return false;
                });
                foreach (IMySlimBlock b in Blocks)
                {
        ...
        public static void Drill(IMyTerminalBlock block, float value, bool find)
        {
            if (find)
            {
                ...
                block.CubeGrid.GetBlocks(Blocks, (s) =>
                {
                    if (s.FatBlock is IMyShipDrill) return true;
                    return false;
                });
                foreach (IMySlimBlock b in Blocks)
                {
        ...
        public static void Clear(IMyCubeGrid grid) {
            List<IMySlimBlock> Blocks = new List<IMySlimBlock>();
            grid.GetBlocks(Blocks, (b) => { if (b.FatBlock != null) return true; return false; });
            foreach(IMySlimBlock B in Blocks){
                if (B.FatBlock is IMyReactor) Overclock.Reactor((IMyTerminalBlock)B.FatBlock, 1, false);
                if (B.FatBlock is IMyGasGenerator) Overclock.GasGenerator((IMyTerminalBlock)B.FatBlock, 1, false);
                if (B.FatBlock is IMyGyro) Overclock.Gyro((IMyTerminalBlock)B.FatBlock, 1, false);
                if (B.FatBlock is IMyThrust) Overclock.Thrust((IMyTerminalBlock)B.FatBlock, 1, false);
                if (B.FatBlock is IMyShipDrill) Overclock.Drill((IMyTerminalBlock)B.FatBlock, 1, false);
            }
        }
    }
}